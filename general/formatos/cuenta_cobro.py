...
class FormatoCuentaCobro():
    ...
    def generar_pdf(self, id):  
        ...
        def draw_header():
            ...
            #Datos factura
            p.setFont("Helvetica-Bold", 9)
            p.drawRightString(x + 500, 720, documento['documento_tipo__nombre'])            
            p.setFont("Helvetica-Bold", 9)
            numero = documento['numero'] if documento['numero'] else ""
            p.drawRightString(x + 475, 710, str(numero))                                   
        ...
        for index, detalle in enumerate(documento_detalles):
            ...
            p.drawString(x + 7, y + alto + 5, str(detalle['item__codigo'])[:13] if detalle['item__codigo'] else "")
