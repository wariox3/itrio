...
class MovimientoViewSet(viewsets.ModelViewSet):
    ...
    def obtener_balance_prueba_tercero(self, fecha_desde, fecha_hasta, cierre = False, cuenta_con_movimiento = False, cuenta_desde = None, cuenta_hasta = None, contacto_id = None):
    ...
    def obtener_estado_situacion_financiera(self, fecha_desde, fecha_hasta):             
        query = f'''            
            select
                MIN(m.id) AS id,
                c.cuenta_clase_id,
            ...
            left join con_cuenta_grupo cg on c.cuenta_grupo_id = cg.id
            where
                m.fecha >= '{fecha_desde}' and m.fecha <= '{fecha_hasta}' 
            group by
                c.cuenta_clase_id,
                cl.nombre,
                c.cuenta_grupo_id,
    ...
    def informe_balance_prueba_tercero(self, request):    
        ...
        resultados_tercero = self.obtener_balance_prueba_tercero(fecha_desde, fecha_hasta, cierre, cuenta_con_movimiento, cuenta_codigo_desde, cuenta_codigo_hasta, contacto_id)
    ...
    @action(detail=False, methods=["post"], url_path=r'informe-estado-situacion-financiera',)
    def informe_estado_situacion_financiera(self, request):    
        raw = request.data
        parametros = raw.get("parametros", [])
        excel = raw.get('excel', False)
        pdf = raw.get('pdf', False)
        fecha_desde = parametros['fecha_desde']
        fecha_hasta = parametros['fecha_hasta']         
        resultados_json = self.obtener_estado_situacion_financiera(fecha_desde, fecha_hasta)
        #if pdf:
        #    formato = FormatoBalancePrueba()
        #    pdf = formato.generar_pdf(fecha_desde, fecha_hasta, resultados_json)
        #    nombre_archivo = f"balance_prueba{fecha_desde}.pdf"
        ...
        if excel:            
            ...
            wb = Workbook()
            ws = wb.active
            ws.title = "estado_situacion_financiera"                                                
            excel_funciones.agregar_titulo(ws, "Estado situacion financiera", "A", "E")                                   
            ws['A4'] = f"Fecha desde: {fecha_desde}"
            ws['A4'].font = excel_funciones.fuente_general           
            ws['A5'] = f"Fecha hasta: {fecha_hasta}"
            ws['A5'].font = excel_funciones.fuente_general 
            ...
            response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            response['Access-Control-Expose-Headers'] = 'Content-Disposition'
            response['Content-Disposition'] = f'attachment; filename=estado_situacion_financiera.xlsx'
            wb.save(response)
            return response
        else:
            return Response({'registros': resultados_json}, status=status.HTTP_200_OK)              